...
class HttpRequest {
   ...
   * @returns A Promise that resolves with the response data, or rejects with a APIError if an error occurs.
   * @throws {APIError} if an error occurs rejects with a APIError
   */
  async send(url, method = "GET", ttl = 60) {
    const fetch = (url2, method2) => {
      return new Promise((resolve, reject) => {
        ...
        const client = (0, https.request)(hostname, options, (res) => {
          ...
          res.on("end", () => {
            ...
            if (encoding === "gzip") {
              buffer = (0, zlib.gunzipSync)(buffer)
            } else if (encoding === "deflate") {
              buffer = (0, zlib.inflateSync)(buffer)
            } else if (encoding === "br") {
              buffer = (0, zlib.brotliDecompressSync)(buffer)
            }
            ...
            if (res.headers["content-type"] === "application/json") {
              try {
                ...
                reject(
                  new error.APIError("Failed to parse response body as JSON")
                )
              }
            } else {
              reject(
          ...
          res.on("error", (err) => {
            reject(new error.APIError(err.message))
          })
        })
    ...
    const cacheKey = (0, crypto.createHash)("md5").update(
      JSON.stringify({
        url,
        method,
        body: this.body,
